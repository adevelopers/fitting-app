...
final class CategoriesViewController: UIViewController {
    ...
    var categoryItems = BehaviorRelay<[CategoryViewItem]>.init(value: [])
    ...
    private func setupSubscriptions() {
        categoryItems
            .bind(to: collectionView.rx.items) { (c, i, e) in
                let cell = c.dequeueReusableCell(withReuseIdentifier: TitleCollectionViewCell.reuseId, for: IndexPath(row: i, section: 0))
                (cell as? TitleCollectionViewCell)?.setup(title: e.text, image: e.image)
                return cell
            }
